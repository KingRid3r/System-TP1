...
//Code Client pour les Sockets avec polling
...
buffer = new char [MAXDATASIZE];
	...
	bzero(&(sockserver.sin_zero), MAXDATASIZE);

	//Creation d'un socket
	int sockserverfd;
	if ((sockserverfd = socket(AF_INET, SOCK_STREAM, 0)) == -1 ) {
		cerr << "Client : erreur creation socket()" << endl;
		exit(EXIT_FAILURE);
	}
	...
	if ( setsockopt(sockserverfd, SOL_SOCKET, SO_REUSEADDR, (void *)&options, (socklen_t) sizeof(options)) == -1 ) {
		cerr << "Client : erreur setsockopts()" << endl;
		close(sockserverfd);
		exit(EXIT_FAILURE);
		}
	...
	if ( ioctl(sockserverfd, FIONBIO, (char *)& options) == -1) {
		cerr << "Client : erreur ioctl()" << endl;
		close(sockserverfd);
		exit(EXIT_FAILURE);
	}

	//liaison (bind) avec le Client
	if( bind(sockserverfd, (struct sockaddr*)& sockserver, sizeof(struct sockaddr)) == -1) {
		cerr << "Client : erreur bind()" << endl;
		close(sockserverfd);
		exit(EXIT_FAILURE);
	}
	...
	if(listen(sockserverfd, BACKLOG) == -1) {
		cerr << "Client : erreur listen()" << endl;
		close(sockserverfd);
		exit(EXIT_FAILURE);
	}
		...
		if ( (pollreturn = poll(fds, nfds, timeout)) < 0 ) {
			...
			if ( fds[0].revents != POLLIN ) {
				...
				if ( (fd_temp = accept(sockserverfd, (struct sockaddr *)&client, &sin_size)) == -1 ) {
					cerr << "Client : erreur accept()" << endl;
					close(sockserverfd);
					exit(EXIT_FAILURE);
				}
				cout << "Client : connexion de " << inet_ntoa(client.sin_addr) << endl;
	...
	while ( message != "FIN" ) {
		...
		if ( (pollreturn = poll(fds, nfds, timeout)) < 0 ) {
			cerr << "Client : erreur poll() réception" << endl;
			close(sockserverfd);
			exit(EXIT_FAILURE);
		}
			...
			if ( (nbrec < 0) && ( errno != EWOULDBLOCK) ) {
				...
				cerr << "Client : erreur connection du client fermée()" << endl;
				close(sockserverfd);
				exit(EXIT_FAILURE);
			}
				...
				message = string(buffer, strlen(buffer) -2);
